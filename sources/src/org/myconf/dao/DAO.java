...
public abstract class DAO extends _DAOBase{
	 ...
	 * @return
	 */
	protected static Number executeNamedStat(String hql){
		return (Number)namedUniqueResult(hql);
	}
	 ...
	 * @return
	 */
	protected static List executeNamedQuery(String hql, int fromIdx, int fetchCount){
		Session ssn = getSession();
		Query q = ssn.getNamedQuery(hql);
		if(fromIdx > 0)
			q.setFirstResult(fromIdx);
	 ...
	 * @return
	 */
	protected static List findNamedAll(String hql){
		return executeNamedQuery(hql, -1, -1);
	}
	 ...
	 * @return
	 */
	protected static Object namedUniqueResult(String hql){
		Session ssn = getSession();
		Query q = ssn.getNamedQuery(hql);
		q.setMaxResults(1);
		return q.uniqueResult();
	...
	protected static int commitNamedUpdate(String hql, int parm1){
